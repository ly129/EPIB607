...
then be expected to answer a series of questions using the
...
collaborative problem solving with peers. The following video gives a
quick overview of the flipped classroom model.

\section*{A focus on computation}\label{a-focus-on-computation}
\addcontentsline{toc}{section}{A focus on computation}

The classic statistics textbooks were written during a time when
computers were still in their infancy. As such, even the newer editions
heavily rely on \emph{by-hand} computations such as looking up tables
for tail probabilities. We take a modern approach and use the computer
...
\item
  ...
  be given as\footnote{Reproduced with permission from
