...
\NormalTok{dat <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{, }\DataTypeTok{each =} \DecValTok{3}\NormalTok{), }\DataTypeTok{y =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{6}\NormalTok{),}\DataTypeTok{each=}\DecValTok{2}\NormalTok{), }\KeywordTok{rep}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{5}\NormalTok{), }\DataTypeTok{each =} \DecValTok{5}\NormalTok{))) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{key =} \StringTok{"variable"}\NormalTok{, }\DataTypeTok{value =} \StringTok{"count"}\NormalTok{)}



\KeywordTok{png}\NormalTok{(}\StringTok{"~/git_repositories/epib607/dalite/002_box_sd_curves/hist.png"}\NormalTok{, }\DataTypeTok{res =} \DecValTok{150}\NormalTok{)}
\KeywordTok{histogram}\NormalTok{(}\OperatorTok{~}\NormalTok{count }\OperatorTok{|}\StringTok{ }\NormalTok{variable, }\DataTypeTok{data =}\NormalTok{ dat, }\DataTypeTok{n =} \DecValTok{6}\NormalTok{, }\DataTypeTok{type =} \StringTok{"count"}\NormalTok{, }\DataTypeTok{xlab =} \StringTok{"integers"}\NormalTok{)}
\KeywordTok{dev.off}\NormalTok{()}

\KeywordTok{bwplot}\NormalTok{(}\OperatorTok{~}\NormalTok{count }\OperatorTok{|}\StringTok{ }\NormalTok{variable, }\DataTypeTok{data =}\NormalTok{ dat, }\DataTypeTok{xlab =} \StringTok{"integers"}\NormalTok{)}

\CommentTok{# arrange the two plots vertically}
\KeywordTok{print}\NormalTok{(p1, }\DataTypeTok{position=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{.6}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{more=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{print}\NormalTok{(p2, }\DataTypeTok{position=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{.4}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
