...
sml.dat$wt6mo <- bf.dat$WEIGHT[bf.dat$time==6]
attach(sml.dat)
...
sml.dat$wt.grp <- wt.grp
write.csv(sml.dat,"bfdataInClassExample.csv",row.names=F)
...
#postscript("bfBarPlot1.eps",horiz=F)
barplot(cbind(table(wt.grp[status==0])/length(wt.grp[status==0])*100,
        table(wt.grp[status==1])/length(wt.grp[status==1])*100),
        xlab="Randomization group",names.arg=c("Control","Intervention"),ylab="%",
        legend.text=c("small","normal-low","normal-high","big"),beside=F) 
dev.off()
#postscript("bfBarPlot2.eps",horiz=F)
barplot(rbind(table(wt.grp[status==0])/length(wt.grp[status==0])*100,
        table(wt.grp[status==1])/length(wt.grp[status==1])*100),
        xlab="Weight category",names.arg=c("small","normal-low","normal-high","big"),ylab="%",
        legend.text=c("Control","Intervention"),beside=T) 
dev.off()
...
#dot plot
stripchart(WEIGHT,xlab="",ylab="Birthweight",main="",col=4,vertical=T)
...
wt.dens <- density(WEIGHT)
wt.dens$x[wt.dens$y==max(wt.dens$y)]
plot(wt.dens$x,wt.dens$y,type="l",lwd=3,xlab="Birthweight",ylab="Proportion")
plot(wt.dens$x,cumsum(wt.dens$y)/sum(wt.dens$y),type="l",lwd=3,xlab="Birthweight",ylab="Cumulative distribution")

#postscript("bfPDFs.eps",horiz=F)
par(mfrow=c(2,2))
wt.dens <- density(WEIGHT,bw=.025)
plot(wt.dens$x,wt.dens$y,type="l",lwd=3,xlab="Birthweight",ylab="Proportion",main="bw=0.025")
wt.dens <- density(WEIGHT,bw=.05)
plot(wt.dens$x,wt.dens$y,type="l",lwd=3,xlab="Birthweight",ylab="Proportion",main="bw=0.05")
wt.dens <- density(WEIGHT)
plot(wt.dens$x,wt.dens$y,type="l",lwd=3,xlab="Birthweight",ylab="Proportion",main="Default bw")
wt.dens <- density(WEIGHT,bw=.25)
plot(wt.dens$x,wt.dens$y,type="l",lwd=3,xlab="Birthweight",ylab="Proportion",main="bw=.25")
dev.off()
...
#mean.wt <- c(3.456545,4.335862,5.236346,6.152530,8.113694,9.553905,10.592621)
age <- c(0,1,2,3,6,9,12)
#postscript("bfTime1.eps",horiz=F)
plot(age,mean.wt,type="l",lwd=2,ylab="Average weight",xlab="Age (months)",ylim=c(0,11))
dev.off()
...
#mean.wt0 <- c(3.446214,4.265519,5.158619,6.076360,8.018286,9.466816,10.536111)
#mean.wt1 <- c(3.465346,4.397129,5.304868,6.218722,8.196484,9.630395,10.641839)

#postscript("bfTime2.eps",horiz=F)
plot(age,mean.wt,type="n",ylab="Average weight",xlab="Age (months)",ylim=c(0,11))
lines(age,mean.wt0,lwd=2,col=3)
lines(age,mean.wt1,lwd=2,col=4)
dev.off()
#postscript("bfTime3.eps",horiz=F)
