...
<<fig.cap="The original data distribution of sampled depths of the ocean. Note that it is bimodal and not Normal looking.", fig.asp=0.718>>=
...
<<fig.cap="The sampling distribution for the mean depth of the ocean with samples of size $n=16$, looks normal (centered at $\\mu=37$ and SD equal to $\\sigma/\\sqrt{16}$) ", fig.asp=0.618, dev='CairoPDF'>>=
plot(Y,already,pch=19,lwd=1,col="white",
type="l",ylim=YLIM, xlim=c(0,max.X),
ylab="Density", xlab=XLAB[panel] )
polygon(c(0,Y),c(0,FREQ),
...
cex=0.85 )
text(0.95*AVE,0.39*YLIM[1],expression(mu), adj=c(0.5,1.25),
cex=0.95 )
...
<<fig.cap="68\\% Confidence interval calculated using  \\mbox{\\texttt{qnorm(p = c(0.16,0.84), 37, 4.2)}}", fig.asp=0.618, dev='CairoPDF'>>=
# with 68% ci
plot(Y,already,pch=19,lwd=1,col="white",
type="l",ylim=YLIM, xlim=c(0,max.X),
ylab="Density", xlab=XLAB[panel] )
...
bty = "n",y.intersp = 2,
pch = c(15,NA,15), lty = c(NA,1,NA), pt.cex = 1.5, lwd = c(NA,2,NA))
# text(AVE*1.05,  0.15*YLIM[1],toString(round(AVE,0)), adj=c(0.5,1),
#      cex=0.85 )
...
<<fig.cap="95\\% Confidence interval calculated using  \\mbox{\\texttt{qnorm(p = c(0.025,0.975), 37, 4.2)}}", fig.asp=0.618, dev='CairoPDF'>>=
# with 68% ci
plot(Y,already,pch=19,lwd=1,col="white",
type="l",ylim=YLIM, xlim=c(0,max.X),
ylab="Density", xlab=XLAB[panel] )
