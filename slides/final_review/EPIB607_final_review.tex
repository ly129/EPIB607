...
\Wider[3em]{
	...
	\begin{center}
		\begin{tabular}{|l|c|c|c|} \hline
			...
			& & &\\
			CI for small $n$  & $\bar{y} \pm$ qt($\cdot$, df=n-1)$\times$SEM & qgamma($\cdot$,shape=c(y,y+1)) & Clopper-Pearson\\
			& & &\\
			CI for large $n$  & qnorm($\cdot$, $\bar{y}$,SEM) & qnorm($\cdot$,$y$,$\sqrt{y}$) & qnorm($\cdot$, $p$, $\sqrt{\frac{p(1-p)}{n}}$)\\
			& & &\\
			$p$value small $n^*$& pt & ppois & pbinom \\
			& & &\\
			$p$value large $n^*$& pnorm & pnorm & pnorm \\
			\hline
		\end{tabular}
