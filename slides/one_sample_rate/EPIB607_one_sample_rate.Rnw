...
\begin{itemize}
		\setlength\itemsep{2em}
	\item If the CLT hasn't kicked in, then the usual CI might not be appropriate: $$\textrm{point-estimate} \pm t^\star  * \textrm{standard error}$$
...
\setlength\itemsep{1.5em}
	...
	\item From this we can calculate the 95\% CI per 100,000 WY for $\lambda$ using a PT=232978 years:
...
\begin{itemize}
	\setlength\itemsep{1.2em}
	\item Cancers in area surrounding the Douglas Point and Pickering nuclear stations
\end{itemize}
